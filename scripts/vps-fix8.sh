#!/bin/bash
# vps-fix8: dogru stop path'i + string-port parse + CDP dogrula
set -u
...
echo "== stop (dogru path: /api/v2/profile/stop/p/ID)"
curl -sk --max-time 30 "$L/api/v2/profile/stop/p/$P" -H "Authorization: Bearer $TOKEN" | head -c 200
echo ""
sleep 5
